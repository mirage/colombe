...
module type VALUE = sig
  ...
  val encode_without_tls : Encoder.encoder -> 'x send -> 'x -> (unit, [> `Protocol of error ]) t
  val decode_without_tls : Decoder.decoder -> 'x recv -> ('x, [> `Protocol of error ]) t
end
  ...
  let encode
    ...
    = fun encoder w v ->
      let fiber : a send -> [> Encoder.error ] Encoder.state = function
        | Payload   ->
          let k encoder = Encoder.write v encoder ; Encoder.write "\r\n" encoder ; Encoder.flush (fun _ -> Encoder.Done) encoder in
...
= struct
    ...
    let rec fiber_write state = function
      | `Response (Some raw) ->
        let rec k raw n =
          ...
          (* XXX(dinosaure): write until raw is non-empty. *)
          if Cstruct.len raw = 0
          then ( if Tls.Engine.handshake_in_progress state
                 then Read { k= k_handshake state; buffer= buffer_with_tls; off= 0; len= 0x1000; }
                 else k_fiber state )
          else Write { k= k raw; buffer= Cstruct.to_string raw; off= 0; len= Cstruct.len raw; } in
        Write { k= k raw; buffer= Cstruct.to_string raw; off= 0; len= Cstruct.len raw; }
  ...
  let rec go_with_tls ctx fiber delayed_data = match ctx.Context_with_tls.tls, fiber, Cstruct.len delayed_data with
    | Some state, fiber, _ when not (Tls.Engine.can_handle_appdata state) ->
      let k state = ctx.tls <- Some state ; go_with_tls ctx fiber delayed_data in
    ...
    | Some state, Write { k; buffer; off; len; }, _ ->
      ...
      | Some (state, raw) ->
        ...
        Write { k; buffer= Cstruct.to_string raw; off= 0; len= Cstruct.len raw; }
      | None -> k len )
    | Some state, Read { k= k_without_tls; buffer= buffer_without_tls; off= off_0; len= len_0; }, 0 ->
      ...
      let rec fiber_read = function
        ...
        | None ->
          ...
          Read { k; buffer= Bytes.make 0x1000 '\000'; off= 0; len= 0x1000; }
  ...
  let starttls_as_client ctx config =
    ...
    let rec k raw len =
      ...
      if Cstruct.len raw = 0
      then handle_handshake ctx state (fun state -> ctx.tls <- Some state ; Return ())
      else Write { k= k raw; buffer= Cstruct.to_string raw; off= 0; len= Cstruct.len raw } in
    ...
    | Sendmail.PLAIN ->
      ...
      | 334 ->
        ...
        ( recv ctx Value.Code >>= function
            ...
            | (code, txts) -> Error (`Tls (`Protocol (`Unexpected_response (code, txts)))) )
...
let pp_error ppf = function
  | `Protocol err
  | `Tls err -> Value_with_tls.pp_error ppf err
  | `Unsupported_mechanism ->
...
let m0 ctx config ?authentication ~domain sender recipients =
  ...
  then Error `STARTTLS_unavailable
    ...
    Value_with_tls.starttls_as_client ctx config |> (reword_error (fun err -> `Tls err)) >>= fun () ->
    let* txts = send ctx Value.Helo domain >>= fun () -> recv ctx Value.PP_250 in
...
let sendmail ({ bind; return; } as impl) rdwr flow context config ?authentication ~domain sender recipients mail : ((unit, error) result, 's) io =
  let ( >>- ) = bind in
  let ( >>= ) x f = x >>- function
    | Ok v -> f v

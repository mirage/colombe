...
module Client = struct
  ...
  let pp_error ppf = function
    | Unsupported_mechanism m -> Fmt.pf ppf "Unsupported mechanism (%a)" pp_mechanism m
    | Authentication_rejected -> Fmt.string ppf "Authentication rejected"
    | Authentication_failed -> Fmt.string ppf "Authentication failed"
    | Weak_mechanism m -> Fmt.pf ppf "Weak mechanism (%a)" pp_mechanism m
    | Line_too_long -> Fmt.string ppf "Line too long"
    | Encryption_required -> Fmt.string ppf "Encryption required"
    | Already_authenticated -> Fmt.string ppf "Already authenticated"
    | err -> pp_error ppf err
  ...
  let next t = match t.q, t.mechanism with
    | `q0, PLAIN ->
      Ok (Encode_request { verb= "AUTH"; args= [ "PLAIN" ] })
    | `q1_plain id, PLAIN ->

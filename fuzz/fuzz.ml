...
let sub_domain = map [ ldh_str ] @@ fun sub_domain ->
...
let path = map [ local; domain_and_address_literal; list domain ] @@ fun local domain rest -> { Colombe.Path.local; domain; rest; }
let reverse_path = option path
let forward_path =
  choose [ const Colombe.Forward_path.Postmaster
...
let everything_expect_crlf = dynamic_bind (range ~min:1 78) (string_from_alphabet everything_expect_crlf)

let command =
  choose [ map [ domain_and_address_literal ] (fun domain -> `Hello domain)
         ; map [ reverse_path; mail_parameters ] (fun reverse_path mail_parameters -> `Mail (reverse_path, mail_parameters))
         ; map [ forward_path; mail_parameters ] (fun forward_path mail_parameters -> `Recipient (forward_path, mail_parameters))
...
let iso1 () =
  ...
  match Colombe.Request.Encoder.to_string v with
  | Error err -> failf "Got an error while encoder: %a." Colombe.Request.Encoder.pp_error err
  | Ok raw ->

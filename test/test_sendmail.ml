...
let rdwr_from_flows inputs outputs =
  ...
  let rec write () bytes off len =
    ...
    | x :: r ->
        ...
        if len < max
        then write () bytes (off + len) (max - len)
        else Unix_scheduler.inj () in
...
let test_0 () =
  ...
  let fiber =
    Sendmail.sendmail unix rdwr () ctx
      ...
      (Colombe_emile.to_reverse_path romain_calascibetta)
      [ Colombe_emile.to_forward_path anil ]
...
let test_3 () =
  ...
  let rdwr, is_empty =
    rdwr_from_flows
      ...
      [ "EHLO gmail.com"; "AUTH PLAIN"; "QUIT" ] in
  let authentication =
    { Sendmail.mechanism = PLAIN; username = "romain"; password = "foobar" }
  in
  let fiber =
  ...
  | Error err ->
      is_empty () ;
      assert (err = `Unsupported_mechanism)
  ...
  let mail =
      ...
      Header.of_list
        Field.
          [
            ...
            Field (Field_name.content_encoding, Encoding, `Bit8);
          ] in
    let hd1 =
      let content1 =
        Content_type.(
    ...
    let header =
      Header.of_list
        Field.
          [
            Field (Field_name.sender, Mailbox, romain_calascibetta);
            Field (Field_name.subject, Unstructured, subject);
            Field (Field_name.v "To", Addresses, [ `Mailbox anil ]);
          ] in
    Mt.make header Mt.multi
      (Mt.multipart ~boundary:"boundary" ~rng:Mt.rng [ part0; part1 ]) in
  let stream =
